    ...
    const fetchTeam = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          setTeamMembers(mapped as unknown as TeamMemberContent[]);
        } else {
          setTeamMembers([]);
        }
      } catch (error) {
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 sm:gap-10 max-w-4xl mx-auto mb-16 sm:mb-24 border-b border-white/10 pb-16">
          {leadership.map(member => (
            <MemberCard key={member._id || member.id || member.name} member={member} onSelect={setSelectedMember} isLeadership={true} />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10 pb-12 pt-4">
          {team.map(member => (
            <div key={member._id || member.id || member.name} className="w-full">
