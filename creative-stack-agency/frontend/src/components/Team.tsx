...
const MemberCard = ({ member, onSelect, isLeadership }: MemberCardProps) => (
  <motion.div 
    whileHover={{ y: -10 }}
    className={`bg-primary p-8 rounded-3xl border ${isLeadership ? 'border-accent/50 shadow-[0_0_20px_rgba(0,212,255,0.15)]' : 'border-white/10'} shadow-xl hover:shadow-[0_0_30px_rgba(0,212,255,0.2)] transition flex flex-col items-center text-center relative overflow-hidden`}
  >
    {isLeadership && (
      <div className="absolute top-0 right-0 bg-accent text-primary font-bold text-sm px-6 py-1.5 rounded-bl-2xl z-10 shadow-lg">
        {member.id === 1 ? 'Founder' : 'Co-Founder'}
      </div>
    )}
    <img 
      loading="lazy" 
      src={member.image} 
      alt={member.name} 
      className={`${isLeadership ? 'w-44 h-44' : 'w-32 h-32'} rounded-full object-cover object-top border-4 border-accent mb-6 shadow-lg`} 
    />
    ...
    <div className="flex gap-4 mt-auto mb-6">
      ...
      {member.social.linkedin && <a href={member.social.linkedin} target="_blank" rel="noopener noreferrer" className="p-2 bg-secondary rounded-full hover:text-accent hover:text-[#0A66C2] transition"><Linkedin size={18} /></a>}
...
export default function Team() {
  ...
  return (
    <section id="team" className="px-8 py-24 bg-secondary text-white font-sans">
      <AnimatePresence>
        {selectedMember && (
      ...
      <h2 className="text-center text-4xl font-bold font-display text-white mb-16">Meet Our Team</h2>
      
      {/* Founders */}
      <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto mb-16 border-b border-white/10 pb-12">
        {leadership.map(member => (
          <MemberCard key={member.id} member={member} onSelect={setSelectedMember} isLeadership={true} />
        ))}
      </div>
