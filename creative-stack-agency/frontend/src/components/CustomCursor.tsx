...
export default function CustomCursor() {
  ...
  return (
    <>
      <motion.div
        className="fixed top-0 left-0 w-8 h-8 rounded-full border border-accent pointer-events-none z-[9999] mix-blend-screen mobile-cursor-hide"
        style={{
          x: smoothX,
          y: smoothY,
      ...
      {bubbles.map((b) => (
        <motion.div
          ...
          className="fixed w-2 h-2 bg-accent rounded-full pointer-events-none z-[9998] blur-[1px] mobile-cursor-hide"
        />
      ))}
    </>
  );
